...
import static io.ipfs.api.cbor.CborConstants.BREAK;
import static io.ipfs.api.cbor.CborConstants.EIGHT_BYTES;
import static io.ipfs.api.cbor.CborConstants.FALSE;
import static io.ipfs.api.cbor.CborConstants.FOUR_BYTES;
import static io.ipfs.api.cbor.CborConstants.NULL;
import static io.ipfs.api.cbor.CborConstants.ONE_BYTE;
import static io.ipfs.api.cbor.CborConstants.TRUE;
import static io.ipfs.api.cbor.CborConstants.TWO_BYTES;
import static io.ipfs.api.cbor.CborConstants.TYPE_ARRAY;
import static io.ipfs.api.cbor.CborConstants.TYPE_BYTE_STRING;
import static io.ipfs.api.cbor.CborConstants.TYPE_FLOAT_SIMPLE;
import static io.ipfs.api.cbor.CborConstants.TYPE_MAP;
import static io.ipfs.api.cbor.CborConstants.TYPE_NEGATIVE_INTEGER;
import static io.ipfs.api.cbor.CborConstants.TYPE_TAG;
import static io.ipfs.api.cbor.CborConstants.TYPE_TEXT_STRING;
import static io.ipfs.api.cbor.CborConstants.UNDEFINED;
...
public class CborEncoder {
   ...
   *
   * @param os the actual output stream to write the CBOR-encoded data to, cannot be <code>null
   *     </code>.
   */
  ...
  /**
   * Interprets a given float-value as a half-precision float value and converts it to its raw
   * integer form, as defined in IEEE 754.
   *
   * <p>Taken from: <a href="http://stackoverflow.com/a/6162687/229140">this Stack Overflow
   * answer</a>.
   *
   ...
   *
   * <p>After calling this method, one is expected to write the given number of array elements,
   * which can be of any type. No length checks are performed.<br>
   * After all array elements are written, one should write a single break value to end the array,
   * see {@link #writeBreak()}.
   *
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * <p>After calling this method, one is expected to write the given number of array elements,
   * which can be of any type. No length checks are performed.
   *
   ...
   *
   * @param bytes the byte string to write, can be <code>null</code> in which case a byte-string of
   *     length <tt>0</tt> is written.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * <p>After calling this method, one is expected to write the given number of string parts. No
   * length checks are performed.<br>
   ...
   *
   * @param value the value to write, values from {@link Double#MIN_VALUE} to {@link
   *     Double#MAX_VALUE} are supported.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * @param value the value to write, values from {@link Float#MIN_VALUE} to {@link Float#MAX_VALUE}
   *     are supported.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
  ...
  /**
   * Writes a signed or unsigned integer value in canonical CBOR format, that is, tries to encode it
   * in a little bytes as possible..
   *
   * @param value the value to write, values from {@link Long#MIN_VALUE} to {@link Long#MAX_VALUE}
   *     are supported.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * @param value the value to write, values in the range <tt>[-4294967296..4294967295]</tt> are
   *     supported.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * <p>After calling this method, one is expected to write any number of map entries, as separate
   * key and value. Keys and values can both be of any type. No length checks are performed.<br>
   * After all map entries are written, one should write a single break value to end the map, see
   * {@link #writeBreak()}.
   *
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   ...
   *
   * <p>After calling this method, one is expected to write any number of map entries, as separate
   * key and value. Keys and values can both be of any type. No length checks are performed.
   *
   ...
   *
   * @param simpleValue the (unsigned byte) value to write, values from <tt>32</tt> to <tt>255</tt>
   *     are supported (though not enforced).
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * <p>Note that this method is <em>platform</em> specific, as the given string value will be
   * encoded in a byte array using the <em>platform</em> encoding! This means that the encoding must
   * be standardized and known.
   *
   * @param value the UTF-8 string to write, can be <code>null</code> in which case an UTF-8 string
   *     of length <tt>0</tt> is written.
   * @throws IOException in case of I/O problems writing the CBOR-encoded value to the underlying
   *     output stream.
   */
   ...
   *
   * @param majorType the major type of the value to write, denotes what semantics the written value
   *     has;
  ...
  /**
   * Encodes and writes an unsigned integer value, that is, tries to encode it in a little bytes as
   * possible.
   *
