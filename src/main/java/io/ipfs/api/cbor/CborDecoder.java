...
import static io.ipfs.api.cbor.CborConstants.DOUBLE_PRECISION_FLOAT;
...
import static io.ipfs.api.cbor.CborConstants.HALF_PRECISION_FLOAT;
import static io.ipfs.api.cbor.CborConstants.NULL;
import static io.ipfs.api.cbor.CborConstants.ONE_BYTE;
import static io.ipfs.api.cbor.CborConstants.SINGLE_PRECISION_FLOAT;
...
import static io.ipfs.api.cbor.CborConstants.TYPE_UNSIGNED_INTEGER;
...
public class CborDecoder {
   ...
   *
   * @param is the actual input stream to read the CBOR-encoded data from, cannot be <code>null
   *     </code>.
   */
  public CborDecoder(InputStream is) {
    ...
    m_is =
        (is instanceof PushbackInputStream)
    ...
    return (len < 0)
        ...
        : (len == ONE_BYTE)
            ...
            : (len == TWO_BYTES)
                ...
                : (len == FOUR_BYTES)
                    ? "four bytes"
                    : (len == EIGHT_BYTES) ? "eight bytes" : "(unknown)";
  }
   ...
   * @throws IOException in case of I/O problems reading the CBOR-type from the underlying input
   *     stream.
   */
   ...
   *
   * @return the number of elements in the array to read, or <tt>-1</tt> in case of infinite-length
   *     arrays.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the read byte string, never <code>null</code>. In case the encoded string has a length
   *     of <tt>0</tt>, an empty string is returned.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the number of bytes in the string to read, or <tt>-1</tt> in case of infinite-length
   *     strings.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the read double value, values from {@link Float#MIN_VALUE} to {@link Float#MAX_VALUE}
   *     are supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the read float value, values from {@link Float#MIN_VALUE} to {@link Float#MAX_VALUE}
   *     are supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the read half-precision float value, values from {@link Float#MIN_VALUE} to {@link
   *     Float#MAX_VALUE} are supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @return the read integer value, values from {@link Long#MIN_VALUE} to {@link Long#MAX_VALUE}
   *     are supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   *
   * @read the small integer value, values in the range <tt>[-4294967296..4294967295]</tt> are
   *     supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     output stream.
   */
   ...
   *
   * @read the small integer value, values from {@link Long#MIN_VALUE} to {@link Long#MAX_VALUE} are
   *     supported.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     output stream.
   */
   ...
   *
   * @return the read UTF-8 encoded string, never <code>null</code>. In case the encoded string has
   *     a length of <tt>0</tt>, an empty string is returned.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
  ...
  /**
   * Reads the next major type from the underlying input stream, and verifies whether it matches the
   * given expectation.
   *
  ...
  /**
   * Reads the next major type from the underlying input stream, verifies whether it matches the
   * given expectation, and decodes the payload into a size.
   *
   * @param majorType the expected major type, cannot be <code>null</code> (unchecked).
   * @return the number of succeeding bytes, &gt;= 0, or <tt>-1</tt> if an infinite-length type is
   *     read.
   * @throws IOException in case of I/O problems reading the CBOR-encoded value from the underlying
   *     input stream.
   */
   ...
   * @throws IOException in case of I/O problems reading the unsigned integer from the underlying
   *     input stream.
   */
   ...
   *
   * @return value the read value, values from {@link Long#MIN_VALUE} to {@link Long#MAX_VALUE} are
   *     supported.
  ...
  protected long readUIntExact(int expectedLength, int length) throws IOException {
    if (((expectedLength == -1) && (length >= ONE_BYTE))
      ...
      fail(
          "Unexpected payload/length! Expected %s, but got %s.",
