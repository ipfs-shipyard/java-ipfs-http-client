...
public class Multipart {
  ...
  public static String createBoundary() {
    ...
    for (int i = 0; i < 32; i++) b.append(allowed.charAt(r.nextInt(allowed.length())));
    return b.toString();
  }
  ...
  public void addSubtree(Path parentPath, NamedStreamable dir) throws IOException {
    ...
    for (NamedStreamable f : dir.getChildren()) {
      if (f.isDirectory()) addSubtree(dirPath, f);
      else addFilePart("file", dirPath, f);
    }
  }

  public void addDirectoryPart(Path path) throws IOException {
    append("--").append(boundary).append(LINE_FEED);
    append("Content-Disposition: file; filename=\"")
        .append(encode(path.toString()))
        .append("\"")
        .append(LINE_FEED);
  ...
  public void addFilePart(String fieldName, Path parent, NamedStreamable uploadFile)
    ...
    Optional<String> fileName =
        uploadFile.getName().map(n -> encode(parent.resolve(n).toString().replace('\\', '/')));
    append("--").append(boundary).append(LINE_FEED);
    if (!fileName.isPresent())
      append("Content-Disposition: file; name=\"")
          .append(fieldName)
          .append("\";")
          .append(LINE_FEED);
    else
      ...
      if (status == HttpURLConnection.HTTP_OK) {
        ...
        throw new IOException(
            "Server returned status: "
                ...
                + b.toString()
                + " and Trailer header: "
                + httpConn.getHeaderFields().get("Trailer"));
      }
