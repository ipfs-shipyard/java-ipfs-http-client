...
public class JSONParser {
  private static char skipSpaces(String json, int[] pos) {
    while (true) {
      if (pos[0] >= json.length()) return 0;
      char ch = json.charAt(pos[0]);
      if (Character.isWhitespace(ch)) pos[0]++;
      else return ch;
    }
  }
  ...
  private static Number parseNumber(String json, int[] pos) {
    ...
    throw new IllegalStateException(
        "Failed to parse JSON number at " + startPos + " '" + numericString + "'");
  }
  ...
  private static List parseArray(String json, int[] pos) {
    ...
    while (true) {
      ...
      if (ch == 0) break;
        ...
        pos[0]++;
        if (skipSpaces(json, pos) == 0) break;
      }
  ...
  private static Object parse(String json, int[] pos) {
    ...
    if (ch == 0) return null;
    int startPos = pos[0];
    if (startPos == json.length()) return null;
  ...
  public static List<Object> parseStream(String json) {
    ...
    json = json.trim();
    while (pos[0] < json.length()) res.add(parse(json, pos));
    return res;
  }
  ...
  private static void toString(Object obj, StringBuffer buf) {
    if (obj == null) buf.append("null");
    else if ((obj instanceof Boolean) || (obj instanceof Number)) buf.append(obj.toString());
      ...
      buf.append(']');
    } else if (obj instanceof String) escapeString(obj.toString(), buf);
    else {
      try {
  ...
  public static String stripWhitespace(String src) {
    ...
    for (int i = 0; i < src.length(); i++) {
      ...
      if (!inQuote) {
        ...
        if (ch == '\\') isEscaped = !isEscaped;
        else if ((ch == '"') && !isEscaped) inQuote = false;
      }
  ...
  public static Object getValue(Object json, String path) {
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      if ((json != null) && (json instanceof Map)) json = ((Map) json).get(key);
      else return null;

      if (index >= 0) {
        if ((json != null) && (json instanceof List)) json = ((List) json).get(index);
        else return null;
      }
    }
